#' Run and saves the Gibbs sampler algorithm results for simulated scenarios or for real datasets
#'    when using initial_values.
#'
...
#' @param initial_val A list with the same structure as the `par.values` argument specifying the initial values
...
sim_random <- function(S, seed, scenario, casenumber, M, N, w, d, as, bs, al, bl, a, b, alpha0, lambda, maxIter,
  ...
  outt <- parallel::parSapply(cls, 1:S, save_sim_random, seed = seed, scenario = scenario,
